...
export function createCatalogModule({
  ...
  async function syncRevCatalog(options = {}) {
    ...
    for (const url of urls) {
      try {
        ...
        if (product.sku && product.name) {
          catalog.set(product.sku.toUpperCase(), product);
    ...
    await fsp.writeFile(path.join(cacheDir, 'rev-catalog.json'), JSON.stringify(Array.from(catalog.values()), null, 2));
    return products;
  }
  ...
  async function loadCachedCatalog() {
    ...
    for (const item of items) catalog.set(item.sku.toUpperCase(), item);
    return items;
  }
  ...
  function getCatalogProduct(sku) {
    return catalog.get(String(sku || '').toUpperCase());
  }
