...
export function registerRoutes(app, {
  ...
  function chatContextFor(project, message, citations = [], catalogHits = []) {
    ...
    return [
      ...
      `Team: ${JSON.stringify(responseProject.team)}`,
      `Season: ${JSON.stringify(responseProject.season)}`,
      `Selected design: ${JSON.stringify(responseProject.concepts?.[1] || responseProject.concepts?.[0])}`,
      `Strategy: ${JSON.stringify(responseProject.strategy)}`,
      `BOM summary: ${JSON.stringify(responseProject.bomSummary)}`,
      `Buy first parts: ${JSON.stringify(responseProject.bomSummary?.buyFirst || [])}`,
      `Physics: ${JSON.stringify(responseProject.physics)}`,
      `CAD: ${JSON.stringify(responseProject.cad)}`,
      `Code files: ${JSON.stringify(responseProject.codeFiles)}`,
      `Autonomous plan: ${JSON.stringify(responseProject.autonomousPlan)}`,
      `Build guide: ${JSON.stringify((responseProject.buildGuide || []).map((step) => ({ phase: step.phase, title: step.title, checkpoint: step.checkpoint, test: step.test })))}`,
      `Driver analysis: ${JSON.stringify(responseProject.driverAnalysis)}`,
      `Grant/sponsor desk: ${JSON.stringify(responseProject.sponsorDesk)}`,
      `Citations available: ${JSON.stringify(citations)}`,
      `Catalog hits: ${JSON.stringify(catalogHits)}`,
    ].join('\n\n');
  }
