...
export function registerRoutes(app, {
  ...
  function chatContextFor(project, message, citations = [], catalogHits = []) {
    const responseProject = projectForResponse(project);
    const selectedDesign = responseProject.concepts?.[1] || responseProject.concepts?.[0] || {};
    const compactContext = {
      team: {
        name: responseProject.team?.name,
        number: responseProject.team?.number,
        experience: responseProject.team?.experience,
        budget: responseProject.team?.budget,
        priorities: responseProject.team?.priorities,
        goals: responseProject.team?.goals,
      },
      season: {
        name: responseProject.season?.seasonName,
        manualVersion: responseProject.season?.manualVersion,
        isSample: responseProject.season?.isSample,
      },
      selectedDesign: {
        name: selectedDesign.name,
        cost: selectedDesign.cost,
        buildTime: selectedDesign.buildTime,
        fit: selectedDesign.fit,
        mechanisms: selectedDesign.mechanisms,
        risks: selectedDesign.risks,
      },
      strategy: {
        recommendation: responseProject.strategy?.recommendation,
        scoringPriorities: responseProject.strategy?.scoringPriorities,
        autonomous: responseProject.strategy?.autonomous,
        teleOp: responseProject.strategy?.teleOp,
        whatToIgnore: responseProject.strategy?.whatToIgnore,
      },
      bom: {
        summary: responseProject.bomSummary,
        topMissing: (responseProject.bom || []).filter((item) => item.missingQty > 0).slice(0, 8).map((item) => ({
      ...
      physics: (responseProject.physics || []).slice(0, 7).map((item) => ({
      ...
      cad: {
        disclaimer: responseProject.cad?.disclaimer,
        robotDimensionsMm: responseProject.cad?.robotDimensionsMm,
        mechanismIds: responseProject.cad?.mechanismIds,
      },
      codeFiles: responseProject.codeFiles,
      autonomousPlan: {
        desiredAction: responseProject.autonomousPlan?.desiredAction,
        reliability: responseProject.autonomousPlan?.reliability,
        sensors: responseProject.autonomousPlan?.sensors,
        path: (responseProject.autonomousPlan?.path || []).map((step) => step.action),
        warnings: responseProject.autonomousPlan?.warnings,
      },
      buildGuide: (responseProject.buildGuide || []).map((step) => ({ phase: step.phase, title: step.title })),
      driverAnalysis: {
        eventCount: responseProject.driverAnalysis?.eventCount,
        suggestions: responseProject.driverAnalysis?.suggestions,
        recommendedMap: responseProject.driverAnalysis?.recommendedMap,
      },
      sponsorDesk: {
        subject: responseProject.sponsorDesk?.subject,
        tiers: responseProject.sponsorDesk?.tiers,
      },
      citations: citations.slice(0, 5),
      catalogHits: catalogHits.slice(0, 3).map((item) => ({ sku: item.sku, name: item.name, price: item.price })),
    };
    return [
      'You are Blueprint, the chat-only AI layer for a deterministic FTC robot demo.',
      'The strategy, design, BOM, physics, CAD, Java code, autonomous plan, build guide, driver log analysis, and grants are already generated by the demo engine. Do not claim you generated new artifacts unless the user asks for chat advice.',
      'Answer as a practical robotics pit mentor. Be concise, specific, and grounded in the provided project context.',
      'Do not make definitive legality claims without citations. If citations are missing, say what to verify.',
      'When suggesting changes, mention which workspace tab to open next.',
      '',
      `User question: ${message}`,
      `Project context: ${JSON.stringify(compactContext)}`,
    ].join('\n\n');
  }
  ...
  app.post('/api/ai/smoke-test', async (_req, res, next) => {
    try {
      res.json(await smokeTestGoogleAiStudio());
    } catch (error) {
      next(error);
    }
  });
  ...
  app.post('/api/projects/:id/generate-blueprint', async (req, res, next) => {
    try {
      ...
      project.selectedDesign = project.concepts[1] || project.concepts[0];
      rebuildDerivedArtifacts(project, { preserveAi: project.generatedBy === 'demo-ai' });
      project.updatedAt = nowIso();
  ...
  app.post('/api/projects/:id/agents/review-plan', (req, res) => {
    ...
    res.json({
      ...
      modelAdapterPayload: {
        ...
        note: 'This endpoint prepares prompts for the model adapter and review workflow. Live generation uses the configured Google AI Studio provider when available.',
      },
  ...
  app.post('/api/projects/:id/chat', async (req, res) => {
    ...
    let answer = '';
    const ai = await streamGoogleAiStudioText({
      systemPrompt: 'You are Blueprint Chat. Use only the provided project context and respond in plain text with short bullets when useful.',
      prompt: chatContextFor(project, message, citations, catalogHits),
      onText: (text) => {
        answer += text;
      },
    });
    if (!ai.ok) {
      return res.status(503).json({
        error: 'Google AI Studio is required for chat responses.',
        detail: ai.error,
        aiStatus: aiStatus(),
      });
    }
    res.json({
      answer: answer || ai.text,
      citations,
      catalogHits,
  ...
  app.post('/api/projects/:id/chat/stream', async (req, res) => {
    ...
    if (!ai.ok) {
      send('error', { error: 'Google AI Studio is required for chat responses.', detail: ai.error, aiStatus: aiStatus() });
    } else {
      send('done', { generatedBy: ai.generatedBy, suggestedActions: chatSuggestionsFor(message, project) });
    }
    res.end();
  });
